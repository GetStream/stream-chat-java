...
> [!NOTE]
> In legacy permission system users can never access users nor channels from other teams. In [Permissions V2](/chat/docs/java/chat_permission_policies/) it is possible to alter this behavior using multi-tenant permissions.
...
```java
var response = User.upsert()
  .user(
    UserRequestObject.builder()
      .id("Jane")
      .role("user")
      .teams(Collections.singletonList("red"))
      .teamsRole(Collections.singletonMap("red", "admin"))
      .build())
  .request();
```
...
In tables below you will find default permission grants for builtin roles that designed for multi-tenant applications. They are useful for [multi-tenant applications](/chat/docs/java/multi_tenant_chat/) only.
...
var response = TeamUsageStats.queryTeamUsageStats().request();

// Query specific month
var response = TeamUsageStats.queryTeamUsageStats().month("2024-01").request();

// Query date range
var response = TeamUsageStats.queryTeamUsageStats()
    .startDate("2024-01-01")
    .endDate("2024-01-31")
    .request();

// With pagination
var response = TeamUsageStats.queryTeamUsageStats().limit(10).request();
if (response.getNext() != null) {
    var nextPage = TeamUsageStats.queryTeamUsageStats()
        .limit(10)
