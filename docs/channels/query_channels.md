...
> [!NOTE]
> For supported query operators, see [Query Syntax Operators](/chat/docs/java/query_syntax_operators/).
...
For the full list of supported query operators, see [Query Syntax Operators](/chat/docs/java/query_syntax_operators/).
...
Map<String, Object> filter = FilterCondition.and(
    FilterCondition.eq("type", "messaging"),
    FilterCondition.in("members", userId),
    FilterCondition.gte("last_message_at", thirtyDaysAgo)
);

ChannelListResponse channels = Channel.list()
    .filterConditions(filter)
    .sort(Sort.builder().field("last_message_at").direction(Direction.DESC).build())
    .limit(20)
    .request();
...
Map<String, Object> broadFilter = FilterCondition.eq("type", "messaging");

// ❌ BAD: Deep nesting of logical operators
Map<String, Object> nestedFilter = FilterCondition.and(
    FilterCondition.or(
        FilterCondition.eq("frozen", true),
        FilterCondition.eq("disabled", true)
    ),
    FilterCondition.or(
        FilterCondition.eq("hidden", true),
        FilterCondition.eq("muted", true)
    )
);
```
...
ChannelListResponse channels = Channel.list()
    .filterCondition("type", "messaging")
    .filterConditions(FilterCondition.in("members", userId))
    .sort(Sort.builder().field("last_message_at").direction(Direction.DESC).build())
    .limit(20)
    .request();
```
