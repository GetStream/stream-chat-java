...
```java
// 1) Filter by type
var filters = new ChannelsBatchFilters();
filters.setTypes(Map.of("$in", List.of("messaging")));

// 2) Filter by specific CIDs
var filtersByCIDS = new ChannelsBatchFilters();
filtersByCIDS.setCids(Map.of("$in", List.of(
...
types.put("$in", List.of("messaging"));
filter.setTypes(types);

var members = List.of(new ChannelBatchMemberRequest("user-123", null));

var resp = updater.addMembers(filter, members).request();

// Update channel data
var updater = Channel.channelBatchUpdater();
var filter = new ChannelsBatchFilters();
Map<String, Object> types = new HashMap<>();
types.put("$in", List.of("messaging", "team"));
filter.setTypes(types);

var data = new ChannelDataUpdate();
data.setFrozen(true);
Map<String, Object> custom = new HashMap<>();
custom.put("color", "blue");
data.setCustom(custom);

var resp = updater.updateData(filter, data).request();
```
...
```java
var taskResponse = TaskStatus.get(response.getTaskId()).request();
```
