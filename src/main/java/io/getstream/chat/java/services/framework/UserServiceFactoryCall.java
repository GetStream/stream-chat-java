 ...
 *
 * <p>This factory creates dynamic proxies around Retrofit service interfaces, intercepting method
 * calls to wrap any {@link retrofit2.Call} results with {@link UserCall}. This enables automatic
 * user token injection for client-side requests without modifying the service interface
 * definitions.
 *
 * <p>The wrapping process is transparent to callers - they interact with the service interface
 * normally, but each Retrofit Call is automatically enhanced with the provided user token.
 *
 * <p><b>Requirements:</b> Service methods must return {@code Call<T>} with a type parameter (not
 * raw Call). The service interface must be compiled with generic type information preserved
 * (default behavior).
 *
 * <p><b>Performance:</b> Response type extraction is cached per-method to minimize reflection
 * overhead on the hot path (~10ns overhead per call after caching vs ~100ns without).
 *
...
final class UserServiceFactoryCall implements UserServiceFactory {
  ...
  /**
   * Cache of response types extracted from service method signatures. Key: Method from service
   * interface Value: Response type T from Call<T> return type
   *
   * <p>Thread-safe and lazily populated on first method invocation.
   */
  ...
  /**
   * Creates a dynamic proxy for the specified service interface that wraps Retrofit Calls with user
   * token context.
   *
   * <p>This method generates a service implementation that intercepts all method calls. ALL service
   * methods MUST return {@link retrofit2.Call} - methods that don't return Call will fail with
   ...
   * @throws IllegalStateException if a service method doesn't return Call or returns raw Call
    ...
    return (TService)
        java.lang.reflect.Proxy.newProxyInstance(
            ...
            (proxy, method, args) -> {
              ...
              Type responseType =
                  responseTypeCache.computeIfAbsent(method, this::extractResponseType);
   ...
   *
   * <p>This method is called once per service method and cached for subsequent invocations.
   *
