 ...
 *
 * <p>This implementation wraps Retrofit service interfaces with a dynamic proxy that intercepts
 * method calls and delegates to {@link UserTokenCallRewriter} for token injection.
 *
 * <p><b>Mechanism:</b> Uses Java reflection {@link java.lang.reflect.Proxy} to wrap the service
...
final class UserServiceFactoryProxy implements UserServiceFactory {
   ...
   *
   * <p>The returned service is a dynamic proxy that intercepts all method calls and delegates to
   * the underlying Retrofit service while injecting the user token.
   *
  ...
  public final <TService> TService create(Class<TService> svcClass, UserToken userToken) {
    return (TService)
        newProxyInstance(
            ...
            new Class<?>[] {svcClass},
            new UserTokenCallRewriter<>(
                retrofit.callFactory(), retrofit.create(svcClass), userToken));
  }
}
