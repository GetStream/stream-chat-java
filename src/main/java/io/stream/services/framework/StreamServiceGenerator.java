...
public class StreamServiceGenerator {
  ...
  public static @NotNull <S> S createService(@NotNull Class<S> serviceClass)
    ...
    if (getApiSecret() == null) {
      StreamException.build(
          "Missing Stream API secret. Please set STREAM_SECRET environment variable or System property");
    }
    if (retrofit == null) {
      int streamChatTimeout = System.getenv("STREAM_CHAT_URL") != null
      ...
      httpClient.addInterceptor(chain -> {
        ...
        HttpUrl url = original.url().newBuilder().addQueryParameter("api_key", getApiKey()).build();
        Request request = original.newBuilder().url(url).header("Content-Type", "application/json")
            .header("X-Stream-Client", "stream-java-client-" + sdkVersion())
            .header("Stream-Auth-Type", "jwt").header("Authorization", jwtToken()).build();
        return chain.proceed(request);
      });
      ObjectMapper mapper = new ObjectMapper();
      mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, failOnUnknownProperties);
      String baseUrl = System.getenv("STREAM_CHAT_URL") != null ? System.getenv("STREAM_CHAT_URL")
  ...
  private static @NotNull String jwtToken() {
    Key signingKey;
    try {
      signingKey = new SecretKeySpec(getApiSecret().getBytes("UTF-8"),
          SignatureAlgorithm.HS256.getJcaName());
      return Jwts.builder().setIssuedAt(new Date()).setIssuer("Stream Chat Java SDK")
          .setSubject("Stream Chat Java SDK").claim("server", true).claim("scope", "admins")
          .setIssuedAt(new Date()).signWith(signingKey, SignatureAlgorithm.HS256).compact();
  ...
  private static @Nullable String getApiSecret() {
    return System.getenv("STREAM_SECRET") != null ? System.getenv("STREAM_SECRET")
        : System.getProperty("STREAM_SECRET");
  }

  private static @Nullable String getApiKey() {
    return System.getenv("STREAM_KEY") != null ? System.getenv("STREAM_KEY")
