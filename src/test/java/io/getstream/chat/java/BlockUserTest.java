    ...
    Assertions.assertDoesNotThrow(
        () -> {
          ...
          var blockedUser = testUsersRequestObjects.get(1);
            BlockUser.BlockUserRequestData.BlockUserRequest blockRequest = BlockUser.blockUser()
            ...
            BlockUser.GetBlockedUsersRequestData.GetBlockedUsersRequest getBlockedUsersRequest = BlockUser.getBlockedUsers(blockingUser.getId());

            BlockUser.GetBlockedUsersResponse getBlockedUsersResponse = getBlockedUsersRequest.request();
            Assertions.assertFalse(getBlockedUsersResponse.getBlockedUsers().isEmpty());
            Assertions.assertEquals(getBlockedUsersResponse.getBlockedUsers().get(0).getBlockedUserID(), blockedUser.getId());
            ...
            Assertions.assertEquals(users.getUsers().get(0).getBlockedUserIDs().get(0), blockedUser.getId());

            // Unblocking the user
            BlockUser.UnblockUserRequestData.UnblockUserRequest unblockRequest = BlockUser.unblockUser()
                .blockedUserID(blockedUser.getId())
                .userID(blockingUser.getId());
            ...
            Assertions.assertTrue(users.getUsers().get(0).getBlockedUserIDs() == null || users.getUsers().get(0).getBlockedUserIDs().isEmpty());
        });
  }
}
